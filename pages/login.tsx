  ...
  const handleLogin = async (username: string, password: string) => {
    try {
      ...
      if (loginResponse.status === 200) {
        ...
        toast.error("Please check your login information again!", {
          autoClose: 3000,
  ...
  return (
    <main className="bg-[url('../public/img/login_bg.png')] bg-center bg-cover">
      ...
      >
        <form
          className="
            max-w-[1280px] min-w-[560px] py-20 my-auto mr-12 rounded-[16px] bg-white
            flex flex-col items-center justify-center gap-6 ml-auto
            max-md:px-12 max-md:mx-auto max-md:min-w-[196px] max-sm:px-10"
          onSubmit={handleSubmit}
        >
          <h1 className="text-base font-bold mb-8 max-md:text-[1.5rem] text-center">
